...
public class MinimalThemeRenderer {

    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ofPattern("dd-MM-yyyy");
    private static final Color TEXT_DARK = new Color(17, 24, 39);
    private static final Color TEXT_MUTED = new Color(107, 114, 128);
    private static final Color RULE_COLOR = new Color(209, 213, 219);
    ...
    public static byte[] render(Invoice invoice, FirmDetails firm, InvoicePrintOptions options) throws Exception {
        ...
        Font firmNameFont = new Font(Font.HELVETICA, 15f * scale, Font.BOLD, TEXT_DARK);
        Font titleFont = new Font(Font.HELVETICA, 12f * scale, Font.BOLD, primary);
        ...
        Font thFont = new Font(Font.HELVETICA, 8f * scale, Font.BOLD, primary);
        Font totalBoldFont = new Font(Font.HELVETICA, 10f * scale, Font.BOLD, primary);
        ...
        // 1. TOP HEADER: Clean Editorial Split
        ...
        if (firm != null) {
            ...
            if (firm.getGstin() != null && !firm.getGstin().isBlank()) {
                leftHeader.addElement(new Paragraph("GSTIN: " + firm.getGstin(), smallMuted));
            }
        }
        ...
        if (invoice != null && invoice.getDueDate() != null) {
            Paragraph pDue = new Paragraph("Due: " + invoice.getDueDate().format(DATE_FMT), smallMuted);
        ...
        doc.add(createDivider(primary, 1.5f));

        // 2. BILLED TO (Minimal Row)
        PdfPTable billToTable = new PdfPTable(new float[]{2.5f, 1.5f});
        billToTable.setWidthPercentage(100);
        billToTable.setSpacingBefore(6f * scale);
        billToTable.setSpacingAfter(8f * scale);

        PdfPCell custCell = new PdfPCell();
        custCell.setBorder(Rectangle.NO_BORDER);
        Customer cust = invoice != null ? invoice.getCustomer() : null;
        custCell.addElement(new Paragraph("BILLED TO", new Font(Font.HELVETICA, 7.5f * scale, Font.BOLD, primary)));
        String custName = cust != null && cust.getName() != null && !cust.getName().isBlank() ? cust.getName().toUpperCase() : "CASH CUSTOMER";
        custCell.addElement(new Paragraph(custName, boldText));
        if (cust != null) {
            if (cust.getAddress() != null && !cust.getAddress().isBlank()) custCell.addElement(new Paragraph(cust.getAddress(), smallMuted));
            if (cust.getPhone() != null && !cust.getPhone().isBlank()) custCell.addElement(new Paragraph("Phone: " + cust.getPhone(), smallMuted));
            if (cust.getGstin() != null && !cust.getGstin().isBlank()) custCell.addElement(new Paragraph("GSTIN: " + cust.getGstin(), boldText));
        }
        billToTable.addCell(custCell);

        PdfPCell emptyRight = new PdfPCell();
        emptyRight.setBorder(Rectangle.NO_BORDER);
        billToTable.addCell(emptyRight);
        doc.add(billToTable);

        // 3. LINE ITEMS (Minimal Table - Horizontal lines only)
        ...
        for (int i = 0; i < itemHeaders.length; i++) {
            ...
            th.setBorderColor(primary);
            th.setBorderWidth(1f);
        ...
        if (items != null && !items.isEmpty()) {
            ...
            for (InvoiceItem it : items) {
                ...
                itemsTable.addCell(makeMinimalCell(String.valueOf(idx++), normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeMinimalCell(itemName, normalText, Element.ALIGN_LEFT, 5f * scale));
                itemsTable.addCell(makeMinimalCell(hsnCode, normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeMinimalCell(String.valueOf(q), normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeMinimalCell(unit, normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeMinimalCell(priceStr, normalText, Element.ALIGN_RIGHT, 5f * scale));
                itemsTable.addCell(makeMinimalCell(amountStr, normalText, Element.ALIGN_RIGHT, 5f * scale));
            }
        } else {
            ...
            empty.setBorderColor(RULE_COLOR);
        ...
        if (firm != null && firm.getBankName() != null && !firm.getBankName().isBlank()) {
            ...
            leftSummary.addElement(new Paragraph("Bank: " + firm.getBankName() + " | A/C: " + (firm.getBankAccount() != null ? firm.getBankAccount() : "-"), smallMuted));
        ...
        addMinimalBreakdownRow(breakdownTbl, "Sub Total", "₹ " + formatAmount(subtotal), normalText, normalText);
        if (discount.compareTo(BigDecimal.ZERO) > 0) {
            addMinimalBreakdownRow(breakdownTbl, "Discount", "- ₹ " + formatAmount(discount), normalText, normalText);
        }
        if (totalTax.compareTo(BigDecimal.ZERO) > 0) {
            addMinimalBreakdownRow(breakdownTbl, "Tax / GST", "₹ " + formatAmount(totalTax), normalText, normalText);
        }

        PdfPCell grandLbl = new PdfPCell(new Phrase("Total Amount", totalBoldFont));
        grandLbl.setBorder(Rectangle.TOP | Rectangle.BOTTOM);
        ...
        grandVal.setBorder(Rectangle.TOP | Rectangle.BOTTOM);
        ...
        if (!isEstimate) {
            addMinimalBreakdownRow(breakdownTbl, "Received", "₹ " + formatAmount(received), normalText, normalText);
            addMinimalBreakdownRow(breakdownTbl, "Balance Due", "₹ " + formatAmount(balance), boldText, boldText);
        }
        ...
        if (showUpiQr) {
            try {
                ...
                qrCell.addElement(new Paragraph("Scan to Pay", smallMuted));
            } catch (Exception ignored) {
    ...
    private static PdfPTable createDivider(Color color, float width) {
    ...
    private static PdfPCell makeMinimalCell(String text, Font font, int alignment, float padding) {
        PdfPCell cell = new PdfPCell(new Phrase(text, font));
        cell.setHorizontalAlignment(alignment);
        cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
        cell.setPadding(padding);
        cell.setBorder(Rectangle.BOTTOM);
        cell.setBorderColor(RULE_COLOR);
        cell.setBorderWidth(0.5f);
        return cell;
    }

    private static void addMinimalBreakdownRow(PdfPTable tbl, String label, String value, Font lFont, Font vFont) {
