...
public class ProfessionalThemeRenderer {

    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ofPattern("dd-MM-yyyy");
    private static final Color TEXT_DARK = new Color(17, 24, 39);
    private static final Color TEXT_MUTED = new Color(75, 85, 99);
    private static final Color DARK_BORDER = new Color(55, 65, 81);
    private static final Color LIGHT_BORDER = new Color(209, 213, 219);
    ...
    public static byte[] render(Invoice invoice, FirmDetails firm, InvoicePrintOptions options) throws Exception {
        ...
        Font bannerTitleFont = new Font(Font.HELVETICA, 14f * scale, Font.BOLD, Color.WHITE);
        Font firmNameFont = new Font(Font.HELVETICA, 13f * scale, Font.BOLD, TEXT_DARK);
        ...
        Font totalBoldFont = new Font(Font.HELVETICA, 9.5f * scale, Font.BOLD, TEXT_DARK);
        ...
        // 1. TOP HEADER BANNER (Full-width colored bar)
        PdfPTable bannerTbl = new PdfPTable(1);
        bannerTbl.setWidthPercentage(100);
        PdfPCell bannerCell = new PdfPCell(new Phrase(docTitle, bannerTitleFont));
        bannerCell.setBackgroundColor(primary);
        bannerCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        bannerCell.setPadding(6f * scale);
        bannerCell.setBorder(Rectangle.NO_BORDER);
        bannerTbl.addCell(bannerCell);
        bannerTbl.setSpacingAfter(8f * scale);
        doc.add(bannerTbl);

        // 2. FIRM INFO & INVOICE DETAILS GRID
        PdfPTable infoGrid = new PdfPTable(new float[]{2.2f, 1.8f});
        infoGrid.setWidthPercentage(100);
        infoGrid.setSpacingAfter(8f * scale);

        // Left: Firm
        PdfPCell leftCell = new PdfPCell();
        leftCell.setBorder(Rectangle.BOX);
        leftCell.setBorderColor(LIGHT_BORDER);
        leftCell.setPadding(8f * scale);
        ...
        if (firm != null && firm.getLogoBase64() != null && !firm.getLogoBase64().isBlank()) {
            try {
                ...
                logo.scaleToFit(70f * scale, 45f * scale);
        ...
        if (firm != null) {
            ...
            if (addr.length() > 0) leftCell.addElement(new Paragraph(addr.toString(), smallMuted));

            StringBuilder contact = new StringBuilder();
            if (firm.getPhone() != null) contact.append("Ph: ").append(firm.getPhone());
            if (firm.getEmail() != null) { if (contact.length() > 0) contact.append(" | "); contact.append(firm.getEmail()); }
            if (contact.length() > 0) leftCell.addElement(new Paragraph(contact.toString(), smallMuted));

            if (firm.getGstin() != null && !firm.getGstin().isBlank()) {
                leftCell.addElement(new Paragraph("GSTIN: " + firm.getGstin(), boldText));
            }
        }
        ...
        rightCell.setBorderColor(LIGHT_BORDER);
        rightCell.setPadding(8f * scale);
        ...
        rightCell.addElement(new Paragraph(numPrefix + invNum, boldText));

        String dateStr = (invoice != null && invoice.getInvoiceDate() != null) ? invoice.getInvoiceDate().format(DATE_FMT) : "-";
        rightCell.addElement(new Paragraph("Date: " + dateStr, normalText));

        if (invoice != null && invoice.getDueDate() != null) {
            rightCell.addElement(new Paragraph("Due Date: " + invoice.getDueDate().format(DATE_FMT), smallMuted));
        }

        // Bill to inside right grid
        Customer cust = invoice != null ? invoice.getCustomer() : null;
        rightCell.addElement(new Paragraph(" ", smallMuted));
        rightCell.addElement(new Paragraph("CUSTOMER (BILL TO):", new Font(Font.HELVETICA, 7.5f * scale, Font.BOLD, primary)));
        String custName = cust != null && cust.getName() != null && !cust.getName().isBlank() ? cust.getName().toUpperCase() : "CASH CUSTOMER";
        rightCell.addElement(new Paragraph(custName, boldText));
        if (cust != null) {
            if (cust.getAddress() != null && !cust.getAddress().isBlank()) rightCell.addElement(new Paragraph(cust.getAddress(), smallMuted));
            if (cust.getPhone() != null && !cust.getPhone().isBlank()) rightCell.addElement(new Paragraph("Ph: " + cust.getPhone(), smallMuted));
            if (cust.getGstin() != null && !cust.getGstin().isBlank()) rightCell.addElement(new Paragraph("GSTIN: " + cust.getGstin(), boldText));
        }

        infoGrid.addCell(rightCell);
        doc.add(infoGrid);

        // 3. ITEMS TABLE (Structured Professional Grid)
        ...
        for (int i = 0; i < itemHeaders.length; i++) {
            ...
            th.setBorderColor(DARK_BORDER);
            th.setBorderWidth(0.5f);
            th.setPadding(5f * scale);
        ...
        if (items != null && !items.isEmpty()) {
            ...
            for (InvoiceItem it : items) {
                ...
                itemsTable.addCell(makeProfCell(String.valueOf(idx++), normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeProfCell(itemName, normalText, Element.ALIGN_LEFT, 5f * scale));
                itemsTable.addCell(makeProfCell(hsnCode, normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeProfCell(String.valueOf(q), normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeProfCell(unit, normalText, Element.ALIGN_CENTER, 5f * scale));
                itemsTable.addCell(makeProfCell(priceStr, normalText, Element.ALIGN_RIGHT, 5f * scale));
                itemsTable.addCell(makeProfCell(amountStr, normalText, Element.ALIGN_RIGHT, 5f * scale));
            }
        } else {
        ...
        leftSummary.setBorder(Rectangle.BOX);
        leftSummary.setBorderColor(LIGHT_BORDER);
        ...
        rightSummary.setBorder(Rectangle.BOX);
        rightSummary.setBorderColor(LIGHT_BORDER);
        rightSummary.setPadding(0);

        PdfPTable breakdownTbl = new PdfPTable(new float[]{1f, 1f});
        breakdownTbl.setWidthPercentage(100);

        addProfBreakdownRow(breakdownTbl, "Sub Total", "₹ " + formatAmount(subtotal), normalText, normalText);
        if (discount.compareTo(BigDecimal.ZERO) > 0) {
            addProfBreakdownRow(breakdownTbl, "Discount", "- ₹ " + formatAmount(discount), normalText, normalText);
        }
        if (totalTax.compareTo(BigDecimal.ZERO) > 0) {
            addProfBreakdownRow(breakdownTbl, "Tax / GST", "₹ " + formatAmount(totalTax), normalText, normalText);
        }

        PdfPCell grandLbl = new PdfPCell(new Phrase("Grand Total", totalBoldFont));
        grandLbl.setBackgroundColor(lightTint);
        grandLbl.setBorderColor(LIGHT_BORDER);
        grandLbl.setBorderWidth(0.5f);
        ...
        grandVal.setBorderColor(LIGHT_BORDER);
        grandVal.setBorderWidth(0.5f);
        grandVal.setPadding(6f * scale);
        grandVal.setHorizontalAlignment(Element.ALIGN_RIGHT);
        breakdownTbl.addCell(grandVal);

        if (!isEstimate) {
            addProfBreakdownRow(breakdownTbl, "Received", "₹ " + formatAmount(received), normalText, normalText);
            addProfBreakdownRow(breakdownTbl, "Balance Due", "₹ " + formatAmount(balance), boldText, boldText);
        }
        ...
        qrCell.setBorderColor(LIGHT_BORDER);
        qrCell.setPadding(6f * scale);
        ...
        if (showUpiQr) {
            try {
                ...
                qrCell.addElement(new Paragraph("SCAN TO PAY (UPI)", new Font(Font.HELVETICA, 7.5f * scale, Font.BOLD, primary)));
        ...
        signCell.setBorderColor(LIGHT_BORDER);
        signCell.setPadding(6f * scale);
    ...
    private static PdfPCell makeProfCell(String text, Font font, int alignment, float padding) {
    ...
    private static void addProfBreakdownRow(PdfPTable tbl, String label, String value, Font lFont, Font vFont) {
